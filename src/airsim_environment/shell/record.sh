...
if [ ! -n "$1" ]
then
    echo "Usage: ./record.sh Environment_name(${Environment_names[@]})"
    exit 1
fi
...
cp ~/guidance_ws/src/airsim_environment/settings/settings_$1_ros.json ~/Documents/AirSim/settings.json
...
sleep 7s
roslaunch airsim_environment 3.launch folder:="$1" & PID4=$!
