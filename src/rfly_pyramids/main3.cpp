...
void imageCb(const sensor_msgs::ImageConstPtr& msg){
	if(!co ){
		...
		ROS_ERROR("cv_bridge exception: %s", e.what());
		return;}
//	Mat I1 = imread("/home/usman/catkin_ws/src/image_pub/Images_I1.tif",0);
//	Mat I2 = imread("/home/usman/catkin_ws/src/image_pub/Images_I2.tif",0);
		Mat flow = flow_calc(cv_ptr_I1->image, cv_ptr_I2->image);
		tform_msg.x = flow.at<float>(0,0);
		tform_msg.y = flow.at<float>(1,0);
		tform_pub.publish(tform_msg);
		cout << flow<<endl;
		//cv::imshow(I1_WINDOW, cv_ptr_I1->image);
		//cv::imshow(I2_WINDOW, cv_ptr_I2->image);
...
// Create a subscriber object
	ros::Subscriber sub = nh.subscribe( "image_publisher" , 10 , &imageCb);//guidance/left_image
// publish the constant velocities on turtle2/cmd_vel
	tform_pub = nh.advertise<geometry_msgs::Point> ( "rfly/pyramids", 10);
   	ros::spin();
 }
